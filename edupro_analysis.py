...
warnings.filterwarnings("ignore")   # suppress noisy pandas/matplotlib warnings

# Create an output folder for charts
os.makedirs("edupro_charts", exist_ok=True)

# A consistent color palette used throughout the script
PALETTE   = "Set2"
FIG_SIZE  = (10, 6)
...
# Quick sanity check — show shape (rows × columns) of each sheet
for name, df in [("Users", users), ("Courses", courses), ("Transactions", transactions)]:
    print(f"  {name:14s}: {df.shape[0]:,} rows × {df.shape[1]} columns")
...
# Create Age Bands — segments used throughout the analysis
def age_band(age):
    """Map a numeric age to a descriptive age-group label."""
    if age < 18:
        return "<18"
    elif age <= 25:
        return "18–25"
...
# Define a display order for age bands (youngest → oldest)
...
print(f"  Gender values         : {users['Gender'].unique().tolist()}")
...
# TransactionDate — already parsed as datetime by pandas/openpyxl; ensure correct dtype
...
print(f"\n  Courses: {len(courses):,}")
print(f"  Categories: {sorted(courses['CourseCategory'].unique().tolist())}")
print()
...
# Merge transactions with users on UserID
merged = transactions.merge(users, on="UserID", how="inner")

# Then merge with courses on CourseID
...
# ─────────────────────────────────────────────
print("=" * 60)
print("STEP 4: Learner Demographic Analysis")
print("=" * 60)

# ── 4a. Age Distribution of ALL registered users ──
...
plt.close()
print("  Saved: 02_learners_by_age_band.png")

# ── 4c. Gender Distribution (all users) ──
...
plt.suptitle("EduPro Learner Gender Distribution", fontsize=14, fontweight="bold", y=1.02)
plt.tight_layout()
...
#     (using merged dataset = actual enrollments)
# ─────────────────────────────────────────────
print("=" * 60)
print("STEP 5: Enrollment Distribution Analysis")
print("=" * 60)

# ── 5a. Enrollments by Course Category ──
...
plt.close()
print("  Saved: 04_enrollments_by_category.png")

# ── 5b. Enrollments by Course Type (Free vs Paid) ──
...
plt.close()
print("  Saved: 05_free_vs_paid.png")

# ── 5c. Enrollments by Course Level ──
level_order  = ["Beginner", "Intermediate", "Advanced"]
level_enroll = (merged["CourseLevel"]
                .value_counts()
...
plt.close()
print("  Saved: 06_enrollments_by_level.png")

print()


# ─────────────────────────────────────────────
# 6.  DEMOGRAPHICS × COURSE PREFERENCE
# ─────────────────────────────────────────────
print("=" * 60)
print("STEP 6: Demographics × Course Preference Analysis")
print("=" * 60)

# ── 6a. Enrollments by Age Band ──
...
plt.close()
print("  Saved: 07_enrollments_by_age_band.png")

# ── 6b. Heatmap — Age Band × Course Category ──
# pivot_table counts enrollments at the intersection of age band and category
...
plt.close()
print("  Saved: 08_heatmap_age_category.png")

# ── 6c. Heatmap — Age Band × Course Level ──
heat_age_lvl = (merged.pivot_table(index="AgeBand", columns="CourseLevel",
                                   values="TransactionID", aggfunc="count", fill_value=0)
                .reindex(AGE_ORDER).fillna(0)
                [level_order]).astype(int)
...
plt.close()
print("  Saved: 09_heatmap_age_level.png")

# ── 6d. Gender × Course Category (stacked bar) ──
...
plt.close()
print("  Saved: 10_gender_category.png")

# ── 6e. Gender × Course Level ──
gender_lvl = (merged.groupby(["Gender", "CourseLevel"])["TransactionID"]
              .count()
              .unstack(fill_value=0)
              [level_order])   # enforce Beginner → Advanced order
...
plt.close()
print("  Saved: 11_gender_level.png")

# ── 6f. Gender × Free vs Paid ──
...
plt.close()
print("  Saved: 12_gender_free_vs_paid.png")
...
# ─────────────────────────────────────────────
print("=" * 60)
print("STEP 7: Behavioral Insights")
print("=" * 60)

# ── 7a. Average courses per learner ──
# How many courses does a typical user enroll in?
courses_per_user = (merged.groupby("UserID")["CourseID"]
                    .nunique()       # count distinct courses per user
...
plt.close()
print("  Saved: 13_courses_per_learner.png")

# ── 7b. Top 10 most enrolled courses ──
...
plt.close()
print("  Saved: 14_top10_courses.png")

# ── 7c. Monthly enrollment trend ──
...
plt.close()
print("  Saved: 15_monthly_trend.png")

# ── 7d. Beginner vs Advanced learner behaviour ──
# Look at which age bands choose Beginner vs Advanced courses
...
ax.set_title("Beginner vs Advanced Enrollments by Age Band",
...
plt.close()
print("  Saved: 16_beginner_vs_advanced_age.png")
...
print(f"""
  ┌─────────────────────────────────────────────────────┐
  │              EduPro KPI Dashboard                   │
  ├─────────────────────────────────────────────────────┤
  │  Total Enrollments          : {total_enrollments:>8,}              │
  │  Unique Active Learners     : {total_unique_learners:>8,}              │
  │  Unique Courses Enrolled    : {total_courses:>8,}              │
  │  Avg Courses / Learner      : {avg_courses:>8.2f}              │
  │  Most Popular Category      : {most_popular_category:<20}   │
  │  Most Popular Level         : {most_popular_level:<20}   │
  │  Most Active Age Band       : {top_age_band:<20}   │""")

for gender, pct in gender_ratio.items():
    print(f"  │  {gender:<28}: {pct:>7.1f}%                │")

print("  └─────────────────────────────────────────────────────┘")


# ─────────────────────────────────────────────
# 9.  SAVE KPI SUMMARY AS A VISUAL CARD
# ─────────────────────────────────────────────
kpi_labels = [
    f"Total Enrollments\n{total_enrollments:,}",
...
for ax, label, color in zip(axes, kpi_labels, colors):
    ...
    ax.text(0.5, 0.5, label, transform=ax.transAxes,
            ...
            color="white", wrap=True, multialignment="center")
    ax.axis("off")

plt.suptitle("EduPro — Key Performance Indicators", fontsize=14,
             fontweight="bold", y=1.05)
plt.tight_layout()
plt.savefig("edupro_charts/00_kpi_summary.png", dpi=150, bbox_inches="tight")
plt.close()
print("\n  Saved: 00_kpi_summary.png")

print()
print("=" * 60)
print("✅  Analysis complete!  All charts saved to ./edupro_charts/")
print("=" * 60)
